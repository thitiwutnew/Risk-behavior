<?php 
   ...
   if(isset($_POST['submit'])){
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST") {
       ...
       $mypassword = mysqli_real_escape_string($connect,md5($_POST['password'])); 
     
       $sql = "SELECT * FROM account WHERE acc_username = '$myusername' and acc_password = '$mypassword'";
       ...
       if($count !=0) {
          ...
          if($row['acc_chk']==1){
            $sqlname = "SELECT * FROM authorities WHERE at_id = ".$row['acc_name']."";
            $resultname = mysqli_query($connect,$sqlname);
            $rowname = mysqli_fetch_array($resultname,MYSQLI_ASSOC);
  
            $_SESSION['login_user'] =  $row['acc_rank']." ".$rowname['at_name'];
            $_SESSION['user_status'] =  $row['acc_status'];
            $_SESSION['user_id'] =  $row['acc_name'];
...
<img src="image/logo.png" style="height: 67px;width: 84px;"><h3  style="color: #fff;    margin-top: 10px;"> ระบบสารสนเทศเพื่อการจัดการความเสี่ยงต่อพฤติกรรมการกระทำผิดกฎหมาย</h3>
  ...
  <div id="formContent">
    ...
    <!-- Icon -->
    <div class="fadeIn first" style="padding: 10px;color: #fff;">
      <b>ล็อกอินเข้าสู่ระบบ</b>
    </div>

    <!-- Login Form -->
    <form  method="POST" enctype="multipart/form-data">
      <input type="text" id="login" class="fadeIn second " name="username" placeholder="ผู้ใช้งาน" autocomplete="off" />
      <input type="password" id="password" class="fadeIn third" name="password" placeholder="รหัสผ่าน" autocomplete="off" />
      <p>
                                <div style="color:white;">
                                    <?php 
                                        if($chk=="ชื่อผู้ใช้งาน  หรือรหัสผ่านไม่ถูกต้อง"){
                                            echo "<center>".$chk."</center>";
                                        }
                                    ?>
                                </div>
                                </p>
      <input type="submit" name="submit" class="fadeIn fourth" value="เข้าสู่ระบบ">
    </form>
  </div>
</div>
      </div>
